...
class PartialFC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dist_multiinput_entropy = DistMultiCrossEntropy()
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(self.fp16):
            embeddings = embeddings.reshape(-1,self.embedding_size) # [n*4,c]
            norm_embeddings = normalize(embeddings)
            norm_weight_activated = normalize(self.weight_activated) # [k,c]
            logits = linear(norm_embeddings, norm_weight_activated) # [n*4,k]
        if self.fp16:
            logits = logits.float()
        ...
        # margin loss
        logits1 = self.margin_softmax(logits.clone(), labels)
        loss0 = self.dist_cross_entropy(logits1, labels)
        
        # add multi-input loss
        logits.mul_(12.0)
        loss1 = self.dist_multiinput_entropy(logits,labels)
...
class DistCrossEntropyFunc(torch.autograd.Function):
    ...
    def forward(ctx, logits: torch.Tensor, label: torch.Tensor):
        ...
        ctx.save_for_backward(index, logits.clone().detach(), label)
...
class DistMultiCrossEntropyFunc(torch.autograd.Function):
    """
    CrossEntropy loss is calculated in parallel, allreduce denominator into single gpu and calculate softmax.
    Implemented of ArcFace (https://arxiv.org/pdf/1801.07698v1.pdf):
    """
    ...
    def forward(ctx, logits: torch.Tensor, label: torch.Tensor):
        ...
        index = torch.where(label != -1)[0]
        # loss
        p_label = torch.zeros(batch_size, 1, device=logits.device)
        p_label[index] = logits.clone()[index].gather(1, label[index])   # label[index] 不在ctx上时,loss为0
        distributed.all_reduce(p_label, distributed.ReduceOp.SUM)

        # scale
        p_label.mul_(10.0)
        p_label = p_label.reshape(-1,4)  # [n,4]
        exp_p_label = p_label.exp()   # herea
        sum_p_exp = torch.sum(exp_p_label,dim=1,keepdim=True) #[n]
        logits1 = exp_p_label / sum_p_exp # [n,4]
        loss = exp_p_label[:,3] / sum_p_exp[:,0] # [n,]

        ctx.save_for_backward(index,logits1, p_label.clone().detach(), logits.clone().detach(), label)
        return loss.clamp_min_(1e-30).log_().mean() * (-1)

    @staticmethod
    def backward(ctx, loss_gradient):
        """
        gradient = -p_i^2(1-p_i) or -p_i^2 * -p_j  if t<3
        gradient = (1-p_i)*p_i * (1-p_i) or (1-p_i)*p_i * -p_j if t == 3
        Args:
            loss_grad (torch.Tensor): gradient backward by last layer
        Returns:
            gradients for each input in forward function
        ...
        scale = torch.tensor([0,0,0,1]).unsqueeze(0).expand_as(p_label).to(logits.device)
        #p_label_g = ((-p_label*scale  + scale) * p_label).reshape(-1,1) # [N*4,1]
        logits1_g = ((-logits1  + scale) * p_label).reshape(-1,1) # [N*4,1]
        ...
        one_hot.scatter_(1, label[index], 1)
        logits[index] -= one_hot
        logits.div_(batch_size)  # [N,K]
        return logits * logits1_g * loss_gradient.item(), None
...
class DistMultiCrossEntropy(torch.nn.Module):
    def __init__(self):
        super(DistMultiCrossEntropy, self).__init__()

    def forward(self, logit_part, label_part):
        return DistMultiCrossEntropyFunc.apply(logit_part, label_part)
