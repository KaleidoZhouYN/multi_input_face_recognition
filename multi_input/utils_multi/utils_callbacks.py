    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        for name in val_targets:
            ...
            if os.path.exists(path):
                ...
                img = data_set[0][0]
                data_set[0][0] = torch.cat((img,img,img),dim=1)
                img = data_set[0][1]
                data_set[0][1] = torch.cat((img,img,img),dim=1)
            ...
            if self.init:
                ...
                if self.writer is not None:
                    ...
                    for i,loss in enumerate(losses):
                        self.writer.add_scalar('loss_{}'.format(i), loss.avg, global_step)
                if fp16:
                    loss_fmt = ','.join(['Loss_{}:{:.5f}'.format(i,loss.avg) for i,loss in enumerate(losses)])
                    msg = "Speed %.2f samples/sec   %s   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Fp16 Grad Scale: %2.f   Required: %1.f hours" % (
                              speed_total, loss_fmt, learning_rate, epoch, global_step,
